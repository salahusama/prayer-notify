...
const AUDIO_WARNING = '../audio/verse-19-59-warning.mp3'
...
const getRandomReminderAudio = () => reminderAudioList[parseInt(Math.random() * 10) % reminderAudioList.length]
...
function sendNotification({ title, message=' ' }, audioFile) {
    ...
    if (audioFile) {
        ...
        notifier.once('click', () => audio.kill())
    }
}
