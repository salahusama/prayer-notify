...
const { sendPrayerNotification, sendReminderNotification, sendStartupNotification } = require('./notify')
...
function scheduleReminderNotification(prevPrayer, nextPrayer, reminderOptions, sendImmediatelyIfPast = false) {
    ...
    if (sendImmediatelyIfPast && msUntilReminder < 0) {
        ...
        sendReminderNotification(prevPrayerName, nextPrayer, reminderOptions.msg)
        return
    }
...
async function schedulePrayerNotificationsForTheDay() {
    ...
    prayers.forEach((prayer, i) => {
        ...
        // Schedule reminder notifications 20 and 60 minutes before this prayer for the previous prayer.
        ...
        if (i > 0 && prayers[i-1].name !== 'Shurooq') {
